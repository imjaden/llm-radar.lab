...
# llm-radar — 双端 Git 分叉自动收敛设计 v1.1

> 版本: v1.1 | 日期: 2026-09-06 | 闭环编号: LLM-RADAR-CL006
> 作者: hermes-personal（personal profile；llm-radar 历史设计文档多为 ops profile 产出，作者字段仅为溯源）
> 变更: v1.0 → v1.1 修复设计评审（documents/reviews/llm-radar-fork-converge-design-review-v1.0-20260906.md, CONDITIONAL PASS 70/100）6 🟡 + 3 🟢 观察
>   - RIG-1: overview.json 规则改用真实字段（顶层 `t` 时间 / `s.pr|pe|to|ll|ho` 计数 / `h` 头条）
>   - RIG-2: 删除「复用 _merge_single」表述，改为明确定义的字段级合并规则
>   - RIG-3: D0 Phase 0 移除硬编码 SHA/计数，以执行时实况为准
>   - RIG-4: D3/D5 显式纳入 _sync_remote/_push_with_recovery docstring 同步
>   - RIG-5: partial 分支收敛复用与参数映射锁定（纳入）
>   - RIG-6: tie-break 改字典序（跨机确定性），删除「平局保留本地」
>   - O-1: 「不丢数据」限定 4 实体维度；hotspots/changelog 声明为并集+有界截断例外
>   - O-2: changelog 折叠同 (type,dimension,id) 多次 update — 接受（display-only）
>   - O-3: commit scope 维持本仓近期主流 `docs@llm-radar`（与早期 docs@design 混用为历史，维持现状）
...
git 层面每次 auto-push commit 只动 3 个 tracked 数据文件（评审实测 8eee136/7d0a990/d0ad26e/e01c6dd 四笔均如此）：

```
data/snapshot.json    全量实体库（providers/people/tools/llms + hotspots + changelog + stats），单文件紧凑 JSON 整行重写
overview.json         站点汇总（顶层 v/t/p/s/h），整行重写
timestamp.json        运行状态（generated_at/server/hostname/period/...），整行重写
```
...
目标：将本地 main（评审时含 8eee136→7d0a990→b981e9e(docs)→a745335(audit@review)）与 origin/main（d0ad26e→08cae7d→e01c6dd）语义合并推回。**执行前 `git fetch origin main` 以当时实况为准**（ahead/behind 数随两侧 cron 持续变化，本文档不承诺固定 SHA；若执行时任一侧已自动/人工收敛，跳过或按实际分叉处理并在审计说明）。
...
8. 验证：
   ...
   - overview.json `s.*` 计数与合并后 snapshot 各维度 len 一致；站点页面数据正常
...
| overview.json | 站点汇总（顶层 `t`=生成时间；`s.pr/pe/to/ll/ho`=各维度计数；`h`=头条热点） | `t` 较新侧胜出；`s.*` 由合并后 snapshot 各维度 len 重算；`p` 周期取较新侧；`h` = 并集去重（按 d+t 完整对）按 d 降序，保留两端头条 |
...
- **4 实体维度（providers/people/tools/llms）：按记录 `id` 并集 —— 数据不丢的唯一性保证（目标 1）**
  ...
  - 两侧都有（同 id）→ 字段级合并，**胜者判定规则**：
    ...
    2. 两侧均非空且不等：若记录含时间类字段（`last_event_date` / `updated_at` / `date`，任一存在）→ 时间较新侧字段胜出
    3. 无时间字段或时间相等（tie）→ 逐字段取 JSON 字典序较大值（字符串按 Unicode 码点比较；数字先转字符串）——**跨机确定性**（不依赖执行机/merge 方向，任何 clone 收敛结果一致）
  - 不复用 `_merge_single()`（L1185，其语义是「参数序 new-wins + 空值填补」，与上述「时间/字典序胜出」不同）；D2 实现内置独立辅助 `_union_record(remote_rec, local_rec)`，规则如上，单测覆盖
...
2. 状态判定：
   ...
   - 否则 → 分叉，继续
3. 守护：工作区有未提交改动（git status --porcelain 非空）→ warn return（调用点保证 clean，此处兜底）
...
| `_auto_push()` partial 分支 L395-416 | 裸 `git push`，失败仅 print_err | **纳入收敛（RIG-5 锁定）**：push 失败 → 同 `_converge_fork()` 兜底；dead-letter 无 changelog → `changelog_count: 0, changelog_snapshot: [], error: 'partial (quality-gate) push converge failed: <stderr>'` |
...
1. D1 并集规则单测：构造两侧 snapshot（local-only / remote-only / 同 id 不同值含时间 / 无时间 tie / hotspots / changelog / stats）断言输出；**真实数据回归**：用 origin/main 与本地 snapshot 跑合并，断言 4 维度计数 ≥ 两侧各自计数（providers =101）、hotspots 截断至 max=90、overview `s.*` 与合并后 len 一致
2. `_union_record` 确定性单测：同输入在「本地为左/右」两次调用结果一致（跨机确定性）；tie 走字典序
3. D2 分支测试（mock `_git_run`）：仅落后 ff / 分叉语义合并成功 / 白名单外冲突 abort 不碰代码 / merge 失败降级 / 收敛后 push 失败降级 / 残留状态清理 / 工作区 dirty 守护
4. D3 回归：`_sync_remote` fork 分支触发收敛；`_push_with_recovery` 冲突分支收敛而非死锁；partial 分支失败走收敛；全 git 调用序列无 `force`
...
2. `docs@llm-radar: git fork auto-converge design v1.1 fix RIG-1~6 (CL006)`（本文档）
3. `feat@llm-radar: auto-converge dual-writer fork (semantic union, D1-D3)`（含单测改写，PASS 后实施）
4. Phase 0 由 ops 执行后产出 `merge@llm-radar: reconcile fork — semantic union (CL006 Phase 0)`（若执行时已无分叉则跳过并在审计说明）
5. 评审/审计产物（review-log、.review-level.yaml、documents/reviews/）由 review profile 按惯例落盘（audit@review:）
