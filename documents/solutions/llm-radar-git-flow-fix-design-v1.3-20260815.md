...
| 数据覆盖风险 | 本地 commit 是完整数据快照(含最新采集)。**注意**: 两机并发时远端数据不"旧"(是对方本轮新采集)——force-push 会暂时丢失对方本轮新实体, 直至对方下轮 `_sync_remote` + `merge_entities` 重新合并。此临时丢失窗口为可接受(数据在下轮收敛), 文档显式承认 |
...
## 待确认清单(已确认 2026-08-15: A1 B1; review RIG-1 已修复)

| 项 | 决策 | 确认结果 |
|----|------|---------|
| A force-with-lease 在 rebase 冲突后的使用 | A1 直接尝试(lease 提供安全边界) | ✅ 采用 |
| B 测试覆盖 | B1 新增 2 个单元测试 | ✅ 采用 |
| RIG-1 数据覆盖语义 | 风险表改为「临时丢失对方本轮新实体, 下轮 _sync_remote + merge_entities 重新合并」 | ✅ 已修复 |
